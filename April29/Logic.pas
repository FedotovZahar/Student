unit  Logic ;

{$mode objfpc}{$H+}

interface
type
  Calculation = record
    x,y: integer; // исходные данные
    f:real;       //результат
   end;

  Tcalcul= array[1..128] of Calculation;


function Culc(var x,y:integer):real;
procedure read_file(var x,y:integer;FileName:string);
procedure save_file(var x,y:integer;FileName:string);
procedure save_dat(var c:Tcalcul; filename: string);
implementation
uses
  Classes, SysUtils;

//функция расчёта выражения:(abs(x)-abs(y))/(1+abs(x*y)
 function Culc(var x,y:integer):real;
  var
    f:real;
   begin
     f:=(abs(x)-abs(y))/(1+abs(x*y));
     f:=Round(f*100)/100;
    Culc:=f;
   end;

//процедура открытия файла
procedure read_file(var x,y:integer;FileName:string);
var F:text;
begin
  assign(f, filename);
  reset(f);
  readln(F, x);
  readln(F, y);
  close(f);
end;
 //процедура сохранения данных
  procedure save_file(var x,y:integer;FileName:string);
  var f: text;
begin
  assign(f, filename);
  rewrite(f);
  writeln(f,(x));
  writeln(f,(y));
  close(f);
end;

  //процедура сохранения отчёта
  procedure save_dat(var c:Tcalcul; filename: string);
  var k:integer;
      s:string;
      F:text;
  begin
    k:=1;
    assign(f, filename);
    Append(f);
    writeln(f,'отчёт о работе');
    writeln(f,'X  Y  F');
    while c[k].x<>0 do
      begin
        s:=(IntToStr(c[k].x)+'   '+IntTostr(c[k].y)+'   '+FloatTostr(c[k].f));
        writeln(f);
        writeln(f,s);
        inc(k);
      end;
    writeln(f);
    close(f);
  end;
end.

